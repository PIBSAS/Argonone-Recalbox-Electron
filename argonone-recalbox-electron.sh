...
if [ ! -f $daemonconfigfile ]; then
	...
	echo '# Argon One Fan Configuration for Recalbox Electron' >> $daemonconfigfile
	echo '#' >> $daemonconfigfile
...
echo '	while True:' >> $powerbuttonscript
echo '		temp = os.popen("cat /sys/class/thermal/thermal_zone0/temp").readline(2)' >> $powerbuttonscript
echo '		temp = temp.replace("temp=","")' >> $powerbuttonscript
#echo '		val = float(temp.replace("'"'"'C",""))' >> $powerbuttonscript
echo '		block = get_fanspeed(temp, fanconfig)' >> $powerbuttonscript
